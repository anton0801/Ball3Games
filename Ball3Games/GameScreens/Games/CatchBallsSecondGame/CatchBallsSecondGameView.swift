...
struct CatchBallsSecondGameView: View {
     ...
     @State var gamescene: CatchBallsSecondGameScene!
     ...
     var body: some View {
         ZStack {
             ...
             if gameOver {
                 if newRecord {
                     ...
                     LoseGameView(currentScore: score, best: recordsVM.game2Record)
                 }
             }
         }
         .onAppear {
             gamescene = CatchBallsSecondGameScene()
         }
         .onReceive(NotificationCenter.default.publisher(for: Notification.Name("game_over")), perform: { notif in
             ...
             self.score = score
             if self.score > recordsVM.game2Record {
                 recordsVM.game2Record = self.score
                 newRecord = true
             }
             livesVM.lives -= 1
